    ...
    override fun onBindViewHolder(holder: MyPaymentVM, position: Int) {
        ...
        holder.b.apply {
           // tvItemRoomType.text = "1 Room"
            ...
            tvMovedStatusPayment.text = String.format("%s %s","Rent for :",model.month)
            ...
            Glide.with(context).load(model.propertyImage).placeholder(R.drawable.available_drawable).into(ivitemPaymentImg)
            when (model.status){
